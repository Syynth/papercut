...
export function setParams(host: Host, changes: Partial<EditorParams>): void {
  const { tool, spriteName, snap, selectMode, selectElement, selectFootprint, selectSize, selectDepth, selectCombine, selectFollowSlopes, selectMatch, selectEverywhere, selectStep, selectBand, selectAnyLayer, ...rest } = changes
  // The host's own fields, Select's among them; whatever was not set is left out, since the command's schema takes no `undefined`.
  const own = Object.fromEntries(Object.entries({ tool, spriteName, snap, selectMode, selectElement, selectFootprint, selectSize, selectDepth, selectCombine, selectFollowSlopes, selectMatch, selectEverywhere, selectStep, selectBand, selectAnyLayer }).filter(([, value]) => value !== undefined))
