...
describe('the placeholder terrain set', () => {
  ...
  it('round-trips through the shape the project file holds and feeds the atlas as-is', () => {
    ...
    const atlas = new TerrainAtlas([{ set: back, image }], { nameOf: (key) => of(key)?.name ?? String(key) })
    expect(atlas.tileFor([GRASS, GRASS, PATH, PATH]).missing).toBe(false)
    expect(atlas.tileFor([STONE, GRASS, SAND, GRASS]).missing).toBe(true)
    expect(atlas.missingReport().map((c) => c.combo)).toEqual(['Grass · Sand · Stone'])
  })
})
