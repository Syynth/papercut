...
/** No voxel here: a SHAPE value (ruling of 2026-09-18), so solidity and paint never share a field. */
export const AIR = -1
...
/**
 * One array, one entry per voxel, indexed by `voxelIndex` in `voxels.ts`:
 * `(y * height + z) * width + x`. A plain number array rather than a typed
 * array so the document is JSON without a serializer.
 *
 * A voxel is its SHAPE and nothing else here (ruling of 2026-09-18). `AIR` is
 * a shape: there is no voxel. What a voxel looks like is not stored on the
 * voxel at all but on each of its faces, as a stack of material layers, in
 * `SurfacePaint.faces` — which is why there is no material array any more,
 * and why erasing paint can never delete geometry.
 */
export interface VoxelData {
  /** A shape (SHAPE_*), or AIR. */
  shape: number[]
}
...
/** A face's four material layers, bottom to top. */
export type MaterialLayers = [Slot, Slot, Slot, Slot]

/** How many material layers a face has. */
export const MATERIAL_LAYERS = 4

/** The slot for a material. */
export function slotOf(material: number): Slot {
  return `m:${material}`
}

/** The material a slot holds, or `null` for an empty slot or one that holds something else. */
export function slotMaterial(slot: Slot | undefined): number | null {
...
export function layersOf(material: number): MaterialLayers {
  return [slotOf(material), null, null, null]
}

/**
 * What the faces of a voxel volume are painted with, and the tint of its cells.
 * Keys are stable grid addresses (see paint.ts).
 *
 * A face with an entry is drawn with its layers; a face with no entry is
 * unpainted and draws the fallback. A face nobody can see has no entry at
 * all: operations that create or remove faces write or remove their stacks
 * (ruling of 2026-09-18), so what is in here is exactly what draws.
 */
export interface SurfacePaint {
  /** `${x},${z},${y},${dir}` -> that face's material layers. */
 ...
 * however their sheets are organised.
 *
 * It has no `side` any more (ruling of 2026-09-18). Every face carries its own
 * material layers, so a cliff says what it is made of itself; what a material
 ...
 * four write shapes — indexed assignment (`voxel.voxels.shape[i] = s`),
 * record assignment (`voxel.paint.faces[key] = layers`), array mutation (`push`,
...
export function createVoxel(width: number, height: number, name = 'Ground', parent: string | null = null, id = newId('vox'), layers = DEFAULT_LAYERS, material = 0): VoxelStructure {
  const count = width * height
  const shape = new Array<number>(count * layers).fill(AIR)
  shape.fill(SHAPE_BLOCK, 0, count)
  // The starting ground is one layer of blocks. Its exposed faces are its tops and the ring of sides
  // around the edge of the volume, and every one of them starts painted: a new map is not magenta.
  const faces: Record<string, MaterialLayers> = {}
  for (let z = 0; z < height; z++) {
    for (let x = 0; x < width; x++) {
      faces[`${x},${z},0,4`] = layersOf(material)
      if (x === width - 1) faces[`${x},${z},0,0`] = layersOf(material)
      if (z === height - 1) faces[`${x},${z},0,1`] = layersOf(material)
      if (x === 0) faces[`${x},${z},0,2`] = layersOf(material)
      if (z === 0) faces[`${x},${z},0,3`] = layersOf(material)
    }
  }
  return {
    id,
