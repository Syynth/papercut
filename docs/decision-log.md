...
## A project's sprites are named regions of its images, standing in for generated ones by name
- **WHEN:** 2026-09-18
- **PROJECT:** papercut
- **SYSTEM:** project format / art
- **SCOPE:** moderate
...
## Placing a transition is a mode of the Tag stage, with its controls in a toolbar on the stage
- **WHEN:** 2026-09-19
- **PROJECT:** papercut
- **SYSTEM:** editor-ui / terrain-tools
...
- **PROJECT:** papercut
- **SYSTEM:** terrain-model
- **SCOPE:** minor/local (refines "A ramp's surface is a tile 1 × 1½ that keeps the surface's corner tags", same day; revises the ramp vocabulary named on 2026-09-17)
- **WHAT:** Where a ramp meets the plateau at its head and the ground at its foot is drawn by the ramp's SURFACE, through the same corner tags and arrangements as anywhere else two things meet: the plateau and the ground are the ramp's neighbours in the dual grid. Head and Foot are not slots. The Ramp archetype's slots are Surface, Rail, and Side if it earns its place.
- **WHY:** Once the surface carries corner tags, a head or a foot is just an edge arrangement of it, and a second way to say the same corner would be a second thing to draw and a second thing to disagree.

## Direction is an axis of every tag, not a slot, and a material says per archetype how many directions its art is drawn for
- **WHEN:** 2026-09-19
- **PROJECT:** papercut
- **SYSTEM:** terrain-model / geometry / editor-ui
- **SCOPE:** architectural
- **WHAT:** DIRECTION IS ITS OWN AXIS of a tag, beside the archetype and apart from the slot, and it is universal: any tag, on any archetype, may name one, and a tag that names none serves every direction. A tag reads material, archetype, direction, slot. Ramps are the first use — the direction a ramp descends toward — and it may have others, such as the way a wall faces. A material has a directions property PER ARCHETYPE, each an enumeration of 1, 2 or 4, that says how many directions its art for that kind of face is drawn for: a material may draw its walls for four directions and its ramps for one. FOUR: a north, east, south and west version, each drawn. TWO: a version that runs down the sheet and one that runs across it, each mirrored for the opposite direction. ONE: a single version, turned by quarter turns for the others. So there is one Surface slot, one Rail, one Side, each with up to four versions; the slot picker lists each slot once and a direction picker beside it offers as many directions as the material's property says for the archetype being worked on. Coverage is shown for the direction being worked in, with the total beside it: a four-direction material owes four times the arrangements. The lookup is the same whatever the property says, specific before general: the art for this direction, then the opposite direction's mirrored, then another's turned, then art that names no direction.
- **WHY:** Steps seen head-on are not steps seen from the side, so some art needs two or four versions; a gravel slope needs one and should not owe three more. Mirroring is right for most stairs but wrong where the art carries a light direction, which is why it is the material's choice and not a rule. Direction is kept off the slot list because it multiplies every slot the same way, and folding it in would turn three slots into twelve entries. It is universal rather than the ramp's because nothing about it is particular to slopes: it is the same kind of qualifier the archetype is, and carrying it on every tag from the start is cheaper than retrofitting it when the second use arrives. Keeping the lookup uniform means changing the property never breaks a map: it only changes what the editor asks for.

## A ramp's surface is 1½ long and drawn from quarter tiles, so sheets stay square
- **WHEN:** 2026-09-19
- **PROJECT:** papercut
- **SYSTEM:** terrain-model / geometry
...
## A double-click on a voxel takes its layer, not its island
- **WHEN:** 2026-09-21
- **PROJECT:** papercut
- **SYSTEM:** select
- **SCOPE:** minor/local
