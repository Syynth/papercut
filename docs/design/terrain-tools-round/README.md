...
| `Rule.dc.html` | **Redrawn 2026-09-21 around the selection.** A verb is one operation on a set of elements: Select gives it the set chosen, with a handle, a ghost and a number; Terrain gives it the set under the brush, clipped to the selection when there is one. What each element offers (voxels: Move, Delete, Fill; faces: Extrude, Flatten, Smooth, Ramp, Paint; edges: Trim, Cut ramp), Extrude along the selected faces' normal, and Edges folding from a mode into a verb. Replaces the board of four modes and one shared Footprint control |
...
| `Questions.dc.html` | **Redrawn 2026-09-21.** The questions the selection raises (where the verbs live, whether a selection masks the brush, mixed normals, what Delete means off voxels, what is selected after a verb), the ones still open from the first round, the ones since answered, and a build order from here: Delete and Fill, then Extrude, then the Terrain brush through the same operations |
